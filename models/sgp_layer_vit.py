"""
code adapted from: 
https://github.com/chenw20/SGPA/blob/main/sgp_layer_vit.py
https://github.com/chenw20/SGPA/blob/main/util.py
"""
...
class SGP_layer(nn.Module):
    def __init__(self, num_heads, max_len, hdim, kernel_type, sample_size, jitter,
                 keys_len, drop_rate, device=None, dtype=None, **kwargs):
        ...
        if kernel_type == 'exponential':
            self.log_sf_exp = nn.Parameter(-4. + 0.* torch.randn((self.num_heads,1), **factory_kwargs)) 
            self.log_ls_exp = nn.Parameter(4. + 1.* torch.randn((self.num_heads,self.dq), **factory_kwargs)) 
        elif kernel_type == 'ard':
            self.log_sf_ard = nn.Parameter(0. + 0.* torch.randn((self.num_heads,1), **factory_kwargs))
            self.log_ls_ard = nn.Parameter(0. + 1.* torch.randn((self.num_heads,self.dq), **factory_kwargs)) 
        else:
        ...
        self.fc_qk = nn.Linear(self.hdim, self.hdim, bias=False, **factory_kwargs)
        if self.kernel_type == 'scale_dot':
            self.fc_k = nn.Linear(self.hdim, self.hdim, bias=False, **factory_kwargs)
        self.fc_v = nn.Linear(self.hdim, self.hdim, bias=False, **factory_kwargs)
        
        self.v = nn.Parameter(torch.randn((self.num_heads, 1, self.keys_len, self.vdim), **factory_kwargs))
        self.s_sqrt_ltri = nn.Parameter(torch.randn((self.num_heads, 1, self.vdim, self.keys_len, self.keys_len), **factory_kwargs))
        self.log_s_sqrt_diag = nn.Parameter(torch.randn((self.num_heads, 1, self.vdim, self.keys_len), **factory_kwargs))
        self.cur_k = nn.Parameter(torch.randn((self.num_heads, 1, self.keys_len, self.hdim), **factory_kwargs))

        self.W_O = nn.Sequential(nn.Linear(self.hdim, self.hdim, **factory_kwargs), nn.Dropout(self.drop_rate))
   
        self.kl_tmps = None
     
    def get_q_k_v_ssqrt(self, x, cur_k, sample=True):
    ...
    def kl_divergence(self):
        if self.kl_tmps is not None:
            s_sqrt_local, v_beta, K_k_beta_k_beta, K_k_beta_k_gamma, chol_K_kk, \
                K_k_gamma_k_gamma, v_gamma, log_ssqrt = self.kl_tmps
        ...
        v2 = torch.linalg.solve_triangular(chol_K_kk, K_k_beta_k_gamma, upper=False)
    ...
    def forward(self, x, cur_k=None, sample=True):
        if cur_k is None:
            cur_k = self.cur_k
        if sample:
            q, k_gamma, k_beta, v_gamma, v_beta, log_ssqrt = self.get_q_k_v_ssqrt(x, cur_k, sample)
        else:
            q, k_gamma, v_gamma = self.get_q_k_v_ssqrt(x, cur_k, sample)
        ...
        if sample:
            ...
            v1 = torch.linalg.solve_triangular(chol_K_kk, K_qk_beta.permute(0,1,3,2), upper=False)
            ...
            # save intermediate results for later kl calculations
            self.kl_tmps = [s_sqrt_local, v_beta, K_k_beta_k_beta, K_k_beta_k_gamma, chol_K_kk, \
                            K_k_gamma_k_gamma, v_gamma, log_ssqrt]

            samples = torch.flatten(samples.permute(0,2,3,1,4),-2,-1) 
            samples = self.W_O(samples).squeeze(1)
            return samples
        else: 

...
class SparseCorrSDE_layer(nn.Module):

    def __init__(self, dim_input, dim_embed, n_inducing, embed_func=None, device=None, dtype=None,
                 inference_mode='marginal', inducing_space='time', r_min=0.01, r_max=0.2, t_max=None, **kwargs):
        ...
        if embed_func is None:
            self.embed_func = nn.Linear(dim_input, dim_embed, **factory_kwargs)
        else:
        ...
        self.log_sigma = nn.Parameter(torch.randn(size=(dim_embed,), **factory_kwargs)*0.1)
        self.param_theta = nn.Parameter(torch.randn(size=(dim_embed,), **factory_kwargs)*0.1 - 1)
        self.max_theta, self.min_theta = -math.log(r_min), -math.log(r_max) 
        # combine the latent linear SDE together - linear weighting
        self.weight = nn.Parameter(torch.empty(size=(dim_embed, dim_embed), **factory_kwargs))
        nn.init.kaiming_uniform_(self.weight, a=math.sqrt(5))
        self.bias = nn.Parameter(torch.empty(size=(dim_embed,), **factory_kwargs))
        ...
        assert self.inducing_space in ['data', 'time']
        Z_init = torch.randn(size=(n_inducing, dim_input), **factory_kwargs)
        if self.inducing_space == 'time':   # time space inducing inputs
            Z_init = self.embed_func(Z_init).detach()
        self.Z = nn.Parameter(Z_init) 
        self.mu_q = nn.Parameter(torch.randn(size=(n_inducing, dim_embed), **factory_kwargs)*0.1)
        # make q distribution also having markov chain structure
        self.lbd_q = nn.Parameter(torch.randn(size=(n_inducing, dim_embed), **factory_kwargs)*0.1)
        self.tau_q = nn.Parameter(torch.randn(size=(n_inducing, dim_embed), **factory_kwargs)*0.1)
    
    def extra_repr(self):
        s = "dim_input={}, dim_embed={}, n_inducing={}, inference_mode={}, inducing_space={}, ".format(\
             self.dim_input, self.dim_embed, self.n_inducing, self.inference_mode, self.inducing_space)
    ...
    @property
    def t_q(self):
        if self.inducing_space == 'data':
            return torch.clamp(self.embed_func(self.Z), min=self.t_min, max=self.t_max)
        if self.inducing_space == 'time':
            return torch.clamp(F.relu(self.Z), min=self.t_min, max=self.t_max)
    ...
    def sort_inducing_points(self, add_endpoints=True, get_var=True):
        ...
        if add_endpoints:
            #t_max = t[-1:] + 5 # essentially make t_max very big
    ...
    def get_interval(self, t, sorted_t=None, mu_q=None, var_q=None, get_var=True):
        # given t of shape (N, D), compute the interval t \in [t_l, t_r] for t_l, t_r \in self.time_q
        # first sort the inducing points, note that sorted_t has shape (M+2, D)
        ...
        ind_t_r = torch.clamp(ind_t_r, min=1, max=self.n_inducing+1).permute(1, 0)
        ind_t_l = ind_t_r - 1
        dt_l = t - sorted_t.gather(0, ind_t_l)
        dt_r = sorted_t.gather(0, ind_t_r) - t
        ...
        diff_mu_q = diff_mu_q.gather(0, ind_t_l)
    ...
    def mean_marginal(self, t):
        #dt_l, dt_r, mu_q_l, mu_q_r, var_dt, _, _, _ = self.get_interval(t, get_var=False)
        ...
        return mean @ self.weight + self.bias
    ...
    def mean_and_var_marginal(self, t):
        ...
        # combine the sdes with linear projection
        mean = mean @ self.weight + self.bias
        var = var @ self.weight.pow(2)
